...
if recordVideoData
    if useMoistureData
        videoFile = VideoWriter('..\results\dmdTemperatureAndMoistureDataWithoutParticles.avi');
    else
        videoFile = VideoWriter('..\results\dmdTemperatureDataWithoutParticles.avi');
    end
    open(videoFile);
    ...
    for ii = 1:ceil(size(temperatureDataWithoutParticles, 2)/plotEveryNthElement)-1
        ...
        subplot(3,3, [4 7])
        heatmapir(geometryXData,geometryYData,temperatureReconstructedWithoutParticles(:,i)-temperatureDataWithoutParticles(:,i),0.1,false);
        colormap(jet); 
        cb = colorbar();
        ylabel(cb,'$\Delta T_\mathrm{DMD} = T_\mathrm{DMD} - T_\mathrm{sim}$','Interpreter','latex', 'FontSize',12,'Rotation', 270);
        % clim([temperatureDataMin-min(temperatureData(:,1)), temperatureDataMax-max(temperatureData(:,1))]); % Add colorbar for temperature
        clim([-4, 4]); % Add colorbar for temperature
        ...
        subplot(3,3, [5 8])
        heatmapir(geometryXData,geometryYData,estimatedStatesCase1(:,i)-temperatureDataWithoutParticles(:,i),0.1,false);
        colormap(jet); 
        cb = colorbar();
        ylabel(cb,'$\Delta T_{KF} = T_{KF} - T_\mathrm{sim}$','Interpreter','latex', 'FontSize',12,'Rotation', 270);
        % clim([temperatureDataMin-min(temperatureData(:,1)), temperatureDataMax-max(temperatureData(:,1))]); % Add colorbar for temperature
        clim([-4, 4]); % Add colorbar for temperature
        title(['KF - Time: ', num2str(round(timeSteps(i),2),'%.02f')]); 
        xlabel('X Spatial Coordinate');
        ylabel('Y Spatial Coordinate');
        % add sensor positions
        hold on;
        ...
        plot(timeSteps(1:i), estimatedMeasurementsCase1(1:i,1),'g--','DisplayName','$\hat y_\mathrm{kf}$');
        ylabel('T(OS) (K)')
        grid on;
        xlim([tStart,tEnd])
        ...
        plot(timeSteps(1:i), estimatedMeasurementsCase1(1:i,2),'g--','DisplayName','$\hat y_\mathrm{kf}$');
        ylabel('T(GS1) (K)')
        grid on;
        xlim([tStart,tEnd])
        ...
        hold on;
        plot(timeSteps(1:i), measurementsReconstructedWithoutParticles(1:i,2),'r--','DisplayName','$\hat y_\mathrm{dmd}$');
        plot(timeSteps(1:i), estimatedMeasurementsCase1(1:i,3),'g--','DisplayName','$\hat y_\mathrm{kf}$');
        ylabel('T(GS2) (K)')
        grid on;
        xlim([tStart,tEnd])
