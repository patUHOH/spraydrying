...
if recordVideoData
    if useMoistureData
        videoFile = VideoWriter('..\results\dmdAndKFTemperatureAndMoistureDataWithParticles.avi');
    else
        videoFile = VideoWriter('..\results\dmdAndKFTemperatureDataWithParticles.avi');
    end
    open(videoFile);
    ...
    for ii = 1:ceil(size(temperatureData, 2)/plotEveryNthElement)-1
        ...
        % clim([-2, 0.5]); % Add colorbar for temperature
        ...
        heatmapir(geometryXData,geometryYData,estimatedStatesCase2(:,i)-temperatureData(:,i),0.1,false);
        colormap(jet); 
        cb = colorbar();
