...
for i = 1:numTimeStepsWithoutParticles-1
    ...
    temperatureDynamicsWithoutParticles(:,i+1) = AtildeTemperature*temperatureDynamicsWithoutParticles(:,i) + BtildeTemperature*[UCase1Temperature(i,:)';humidityDynamicsWithoutParticles(:,i)];
    humidityDynamicsWithoutParticles(:,i+1) = AtildeHumidity*humidityDynamicsWithoutParticles(:,i) + BtildeHumidity*[UCase1Humidity(i,:)';temperatureDynamicsWithoutParticles(:,i)];
end
numTimeStepsWithParticles = size(temperatureData,2);
% iterate over time steps
for i = 1:numTimeStepsWithParticles-1
    % compute reduced order dynamics
    temperatureDynamicsWithParticles(:,i+1) = AtildeTemperature*temperatureDynamicsWithParticles(:,i) + BtildeTemperature*[UCase2Temperature(i,:)';humidityDynamicsWithParticles(:,i)];
    humidityDynamicsWithParticles(:,i+1) = AtildeHumidity*humidityDynamicsWithParticles(:,i) + BtildeHumidity*[UCase2Humidity(i,:)';temperatureDynamicsWithParticles(:,i)];
end
